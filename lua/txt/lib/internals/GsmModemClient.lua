...
function GsmModemClient:send_message(message)
    ...
    if      status <= 199 then return { outcome = "error", message = "Informational?! Something is wrong." }
    elseif  status <= 299 then return { outcome = "ok", message = "OK, message sent." }
    elseif  status <= 399 then return { outcome = "error", message = "Redirection?! Something is wrong." }
    elseif  status <= 499 then return { outcome = "error", message = "Client Error. Check phone config." }
    elseif  status <= 599 then return { outcome = "error", message = "Server Error. Check phone config." }
    end
end
